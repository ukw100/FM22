...
static uint_fast16_t                uart_rxstart = 0;                           // head, not volatile
...
#define UART_USART_CLOCK_CMD        RCC_APB1PeriphClockCmd
#define UART_USART_CLOCK            RCC_APB1Periph_UART4
...
#define UART_PREFIX_INPUT_FLUSH     UART_CONCAT(UART_PREFIX, _input_flush)
 ...
 */
void
UART_PREFIX_INIT (uint32_t baudrate, uint16_t wordlength, uint16_t parity, uint16_t stopbits)
{
    ...
    if (last_baudrate != baudrate)
    {
...
#else
        gpio.GPIO_PuPd  = GPIO_PuPd_UP;                                                             // fm TODO: GPIO_PuPd_UP
#endif
        ...
        uart.USART_WordLength           = wordlength;                           // USART_WordLength_8b or USART_WordLength_9b;
        uart.USART_Parity               = parity;                               // USART_Parity_No, USART_Parity_Even, or USART_Parity_Odd
        uart.USART_StopBits             = stopbits;                             // USART_StopBits_1, USART_StopBits_0_5, USART_StopBits_1, USART_StopBits_2, or USART_StopBits_1_5
...
{
    static uint_fast16_t uart_txstop  = 0;                                      // tail
...
/*---------------------------------------------------------------------------------------------------------------------------------------------------
 * uart_input_flush () - throw away input characters
 *---------------------------------------------------------------------------------------------------------------------------------------------------
 */
void
UART_PREFIX_INPUT_FLUSH (void)
{
#if 0
    if (uart_rxsize != 0)                                                       // rx buffer empty?
    {                                                                           // no...
        __disable_irq();
        uart_rxsize = 0;                                                        // reset size
        __enable_irq();
    }
#else
    uint_fast8_t    buf[1];

    while (uart_rxsize)                                                         // rx buffer empty?
    {
        UART_PREFIX_POLL (buf);
    }
#endif
}
...
void UART_IRQ_HANDLER (void)
{
    ...
    {
        static uint_fast16_t uart_txstart = 0;                                  // head
